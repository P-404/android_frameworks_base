...
public class NetworkTrafficSB extends NetworkTraffic implements DarkReceiver, StatusIconDisplayable {
    ...
    protected void setMode() {
        super.setMode();
        mIsEnabled = mIsEnabled && !hasNotch();
    }
    ...
    protected RelativeSizeSpan getSpeedRelativeSizeSpan() {
        return new RelativeSizeSpan((mShowDynamic) ? 0.75f : 1f);
    }

    @Override
    protected RelativeSizeSpan getUnitRelativeSizeSpan() {
        return new RelativeSizeSpan((mShowDynamic) ? 0.7f : 0.9f);
    }
    ...
    private boolean hasNotch() {
        ...
        resid = mContext.getResources().getIdentifier("config_fillMainBuiltInDisplayCutout",
                "bool", "android");
        if (resid > 0) {
